...
for entry in "${entries[@]}"; do
    ...
    for window_title in $open_windows; do
        if [[ $window_title =~ $regex ]]; then
            ...
            # Check if the regex contains special characters
            if [[ $regex =~ [.*\[\](){}+^$\\|] ]]; then
                echo "Regex contains special characters"
                # Perform action for windows matching regex with special characters
                # shellcheck disable=SC2046
                kill -STOP $(pidof "$window_title")
            else
            # Get the PID associated with the current window class
                additional_matches=$(xdotool search --class "$window_title")
                # echo "Additional kill: $window_title pid: $additional_matches" 
                # Pause the process
                for match in $additional_matches; do 
                    echo "Additionall kill: $match"
                    kill -STOP "$additional_matches";
                done 
...
#                    pids="$(xdotool search --class $cur| xargs)"
#                     for pid in $pids; do
#                         killall -STOP "$pid"
#                     done
